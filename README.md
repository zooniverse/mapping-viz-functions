...
The current schema is saved in db/schema.sql. This file is generated using [pg_dump](https://www.postgresql.org/docs/9.1/app-pgdump.html) (version 9.5.23). There are also files containing test data for local development, and the full dataset for back-up purposes - those live in db/data. The full_data.sql file is compressed using the `pg_dump` command's built in compression (using the flag `--compress=9`).
